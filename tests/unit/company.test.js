...
const AXON_SOFT_ANAF_RECORD = {
  cui: 13049596,
  name: 'AXON SOFT SRL',
  address: 'Str. NASAUD, 14, Municipiul Cluj-Napoca, Cluj',
  caenCode: '5829',
  inactive: false,
  vatRegistered: true,
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch Axon Soft via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(AXON_SOFT_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'AXON SOFT SRL');
      expect(result).toHaveProperty('cif', '13049596');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('AXON SOFT SRL');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 13049596, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('13049596');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(AXON_SOFT_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'AXON SOFT SRL' }]));
    ...
    if (AXON_SOFT_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...AXON_SOFT_ANAF_RECORD, inactive: true };
